...
func Init() {
	...
	rExpiredTime, err := strconv.Atoi(os.Getenv("REFRESH_TOKEN_TIME"))
	if err != nil {
	...
	PKG = &jsonWebToken{
		...
		RExpiredTime: time.Duration(rExpiredTime) * time.Hour,
	}
}
...
func (j *jsonWebToken) CreateRefreshToken(userId uuid.UUID) (string, error) {
	claim := &Claims{
		UserId: userId,
		RegisteredClaims: jwt.RegisteredClaims{
			ExpiresAt: jwt.NewNumericDate(time.Now().Add(j.RExpiredTime)),
		},
	}
...
func (j *jsonWebToken) ValidateRefreshToken(tokenString string) (uuid.UUID, error) {
	var (
		claims Claims
		userId uuid.UUID
	)
